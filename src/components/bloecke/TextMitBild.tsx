  ...
  return (
    <section className="abschnitt">
      <div className="container-seite grid items-center gap-10 lg:grid-cols-2 lg:gap-20 2xl:gap-28">
        <div className={cn('relative aspect-[4/3] overflow-hidden rounded-[var(--radius-karte)] border border-akzent/20 bg-flaeche', bildLinks ? 'lg:order-1' : 'lg:order-2')}>
        ...
        <div className={cn('max-w-3xl', bildLinks ? 'lg:order-2' : 'lg:order-1')}>
          ...
          <div className="lesebreite mt-6 space-y-5 text-text-leise">
            {absaetze(d.text).map((absatz, i) => (
              <p key={i}>{absatz}</p>
            ))}
          </div>
          {d.knopf.text && d.knopf.link ? (
            <Link href={d.knopf.link} className="knopf-sekundaer mt-8">
              {d.knopf.text}
            </Link>
          ) : null}
        </div>
